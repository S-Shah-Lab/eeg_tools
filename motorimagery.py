...
if __name__ == '__main__':
    ...
    parser.add_argument( '-pause', metavar='pause',      type=float, default=0.5,             help="how long do you want to wait for each plot?")
    parser.add_argument( '-pdf',   metavar='pdf',        type=bool,  default=False,           help="do you only want to generate the pdf report?")
...
if __name__ == '__main__':
    ...
    pause = opts.pause                               # how long do you want to wait for each plot?
    pdf_only = opts.pdf                              # do you only want to generate the pdf report?
    ...
    path_to_folder = EEG.clean_path(EEG.create_folder(path=file_path, folder_name=base_name))
    ...
    if not pdf_only:
        ...
        for i in range(N):
            ...
            if i==0: 
                ...
                axes2.text(-0.5, 1.21, 'Target (O)', va='bottom', ha='right', transform=axes2.transAxes, color='limegreen', fontsize=12)
                axes2.text(-0.5, 1.2, 'Target (O)', va='bottom', ha='right', transform=axes2.transAxes, color='black', fontsize=12)
                axes2.text(1.5, 1.21, 'Interpolated (X)', va='bottom', ha='left', transform=axes2.transAxes, color='darkgrey', fontsize=12)
        ...
        for ch in chs:
            ...
            for k, mode in enumerate(['Left', 'Right']):
                ...
                figTopo.savefig(f'{path_to_folder}topoR_{ch}_{mode}_{resolution}_Hz.svg', bbox_inches='tight')

                figPSD.savefig(f'{path_to_folder}psd_{ch}_{mode}_{resolution}_Hz.png', bbox_inches='tight')
                figPSD.savefig(f'{path_to_folder}psd_{ch}_{mode}_{resolution}_Hz.svg', bbox_inches='tight')

                figCorr.savefig(f'{path_to_folder}corr_{ch}_{mode}_{resolution}_Hz.png', bbox_inches='tight')
                figCorr.savefig(f'{path_to_folder}corr_{ch}_{mode}_{resolution}_Hz.svg', bbox_inches='tight')
        ...
        min_ = np.min(EEG.convert_dB(x[:, ch_setSLAP.find_labels(chs), :]))
        max_ = np.max(EEG.convert_dB(x[:, ch_setSLAP.find_labels(chs), :]))
        ...
        for ch in chs: 
            ...
            PLOT.plot_frequency_bands(ax=ax1, ylim=(min_, max_), fontsize=12, fraction=0.07)
            
            ax2 = fig.add_subplot(gs[1, 0])
            x_ = np.vstack([x_left_rest, x_left])
            isTreatment = np.arange(x.shape[0]) < x_left_rest.shape[0]
            ax2.plot(STAT.bins, STAT.CalculateR2(EEG.convert_dB(x_), isTreatment, signed=False), color='blue', label='Left')
            
            x_ = np.vstack([x_right_rest, x_right])
            isTreatment = np.arange(x.shape[0]) < x_right_rest.shape[0]
            ax2.plot(STAT.bins, STAT.CalculateR2(EEG.convert_dB(x_), isTreatment, signed=False), color='red', label='Right')
            ...
            fig.savefig(f'{path_to_folder}{ch}_PSDs_{resolution}_Hz.svg', bbox_inches='tight')
            plt.show(block=False)
            plt.pause(pause)
            plt.close()
