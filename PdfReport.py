...
class generate_pdf:
	def __init__(self, plot_folder, montage_name, resolution):
		...
		# Subject's condition
		if 'HC' in self.sub_name: self.condition = 'HC'
		elif 'BI' in self.sub_name: self.condition = 'TBI'
		else: self.condition = 'N/A'
		...
		self.top_left =     (self.move_right_by(xmin, delta), self.move_down_by(ymin, delta))
		self.top_right =    (self.move_left_by(xmax, delta),  self.move_down_by(ymin, delta))
		self.bottom_right = (self.move_left_by(xmax, delta),  self.move_up_by(ymax, delta))
		self.bottom_left =  (self.move_right_by(xmin, delta), self.move_up_by(ymax, delta))
		...
		self.show_key_point('A', x=self.top_left[0],     y=self.top_left[1],     show=False)  # Top left (A)
		self.show_key_point('B', x=self.top_right[0],    y=self.top_right[1],    show=False)  # Top right (B)
		self.show_key_point('C', x=self.bottom_right[0], y=self.bottom_right[1], show=False)  # Bottom right (C)
		self.show_key_point('D', x=self.bottom_left[0],  y=self.bottom_left[1],  show=False)  # Bottom left (D)
		...
		self.draw_hline(x1=self.top_left[0], x2=self.top_right[0], y=self.move_down_by(self.top_left[1], gap_from_top), color=colors.black)
		# Create additional anchor points 
		self.show_key_point('E', x=self.top_left[0],                                        y=self.move_down_by(self.top_left[1], gap_from_top), show=False)
		self.show_key_point('F', x=self.top_right[0],                                       y=self.move_down_by(self.top_left[1], gap_from_top), show=False)
		...
		self.show_key_point('G', x=self.top_left[0],                                        y=self.move_down_by(self.top_left[1], gap_from_top), show=False)
		self.show_key_point('H', x=self.top_right[0],                                       y=self.move_down_by(self.top_left[1], gap_from_top), show=False)
		...
		dx = 10
		dy = 15
		self.write_text(['Motor Imagery Report'],           self.xmid,         self.move_down_by(self.dict_alph['I'][1], y0), font_size=17, bold_flags=[True],  align='center')
		self.write_text([f'Report: {self.formatted_date}'], self.top_right[0], self.move_down_by(self.dict_alph['I'][1], y0), font_size=11, bold_flags=[False], align='right')
		
		font_size=11
		height = self.move_down_by(self.dict_alph['I'][1], 15) - self.move_up_by(self.dict_alph['J'][1], 5)
		dy = height / 3
		# Left side
		self.write_text(['Subject: ',            f'{self.sub_name}'],   self.xmid-dx, self.move_down_by(self.dict_alph['I'][1], y0 + dy  ), font_size=font_size, bold_flags=[False, True], align='right')
		self.write_text(['Session: ',            f'{self.ses_name}'],   self.xmid-dx, self.move_down_by(self.dict_alph['I'][1], y0 + 2*dy), font_size=font_size, bold_flags=[False, True], align='right')
		self.write_text(['Date of Assessment: ', f'{self.date_test}'],  self.xmid-dx, self.move_down_by(self.dict_alph['I'][1], y0 + 3*dy), font_size=font_size, bold_flags=[False, True], align='right')
		# Right side
		self.write_text(['Age: ',                  f'{self.sub_age} '],                         self.xmid+dx, self.move_down_by(self.dict_alph['I'][1], y0 + dy  ), font_size=font_size, bold_flags=[False, True], align='left')
	...
	def write_text(self, text_list, x, y, font_size=12, bold_flags=None, align='center'):
	...
	def draw_svg_image(self, image_path, x, y, width=None, height=None):
	    ...
	    svg_width_pt, svg_height_pt = float(svg_dimensions[0].split('pt')[0]), float(svg_dimensions[1].split('pt')[0])
	    ...
	    if height:
	        ...
	        raise ValueError("Either width or height must be provided to scale the SVG.")
