...
# ///////////////////////////////////////////////////////////////////////////
# $Id: Electrodes.py 7372 2023-05-23 20:39:07Z jhill $
...
class ChannelSet(numpy.matrix):
	...
	def rasterplot(self, img, x=None, y=None, interactive=False, drawnow=True, **imgArgs):
		...
		def select( channelIndex, paramValue ):
			...
			paramIndex = numpy.argmin( numpy.abs( numpy.array( h.paramValues ) - paramValue ).ravel() )
			...
			if len( h.channels ) > 1:
				...
				plt.title( '%s = %g' % ( paramAxisLabel.split('\n')[ 0 ], paramValue ) ) # TODO
			
			plt.axes( h.paramAxes )
			Plotting.plot( h.paramValues, evolution, drawnow=False)
			plt.plot( [ paramValue ] * 2, [ 0, 1 ], transform=h.paramAxes.get_xaxis_transform(), clip_on=False, linewidth=2, alpha=0.7 )
			plt.gca().set( ylim=h.get_clim(), xlim=( h.paramValues[ 0 ], h.paramValues[ -1 ] ), xlabel=paramAxisLabel, title='%d: %s' % ( channelIndex, channelInstance ) )
	...
	def RerefMatrix(self, newReferenceLabels, exclude='auto', keep='all', filters_as='columns', coords=None):
		"""
		...
		neg[exclude, :] = 0.0
		neg[:, exclude] = 0.0
		pos[ newReferenceIndices[ 0 ], newReferenceIndices[ 0 ] ] = 0 # makes it invertible; retains the previous reference as a channel
		W = pos - neg
		W = W[:, keep]
